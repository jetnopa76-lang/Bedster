...
export default function App() {
  ...
  const createBatch = () => {
    ...
    const b = { id: ++_b, name: `Batch ${_b}`, templateId: templates[0].id, substrate: substrates[0] || "Glossy", images: [] };
  ...
  const genPDF = async (batch) => {
    ...
    try {
      ...
      const payload = {
        trayW: tmpl.pageW,
        trayH: tmpl.pageH,
        filename: `${batch.name.replace(/\s/g, "_")}_${tmpl.sizeLabel.replace(/[^a-zA-Z0-9]/g, "")}_${batch.substrate}.pdf`,
      ...
      dlBlob(blob, payload.filename);
      setStatus(`Done! ${filled.length} photos placed.`);
