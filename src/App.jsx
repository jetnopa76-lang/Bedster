...
/* ── Resize image for API upload (stay under Vercel 4.5MB body limit) ── */
function resizeImageForAPI(dataUrl, maxW, maxH) {
  return new Promise((resolve) => {
    const img = new Image();
    img.onload = () => {
...
export default function App() {
  ...
  const genPDF = async (batch) => {
    ...
    try {
      ...
      // Resize images for API (Vercel has 4.5MB body limit)
      // Target: 300 DPI for the frame size, JPEG quality 85
      setStatus(`Compressing ${filled.length} images...`);
      const compressedPlacements = [];
      for (const p of filled) {
        const maxW = Math.ceil(p.w * 300);
        const maxH = Math.ceil(p.h * 300);
        const compressed = await resizeImageForAPI(p.image.dataUrl, maxW, maxH);
        compressedPlacements.push({
      ...
      setStatus(`Uploading ${filled.length} images...`);
      const payload = {
        trayW: tmpl.pageW,
        trayH: tmpl.pageH,
