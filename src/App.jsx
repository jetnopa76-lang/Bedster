...
async function parseIDML(arrayBuffer) {
  ...
  for (const sp of spreadFiles) {
    ...
    doc.querySelectorAll("Rectangle").forEach(rect => {
      ...
      let spreadLeft, spreadTop;
      if (t[3] === -1) { spreadLeft = t[0] * minX + t[4]; spreadTop = -maxY + t[5]; }
      else { spreadLeft = t[0] * minX + t[4]; spreadTop = t[3] * minY + t[5]; }
      imageFrames.push({ label, x: (spreadLeft - pageSpreadLeft) / 72, y: (spreadTop - pageSpreadTop) / 72, w: fw, h: fh });
    });
...
export default function App() {
  ...
  // Build half-page traveler PDF with QR code for sticker generation
  function buildTravelerPDF(batch, tmpl, filledCount, stickerUrl) {
    const pw = 8.5 * 72, ph = 5.5 * 72; // half letter
    const parts = []; let pos = 0; const offsets = {};
    ...
    // Generate QR code as PDF drawing commands
    const qrModules = generateQRModules(stickerUrl);
    const qrSize = 90; // points
    const qrX = pw - 36 - qrSize;
    const qrY = ph - 32 - qrSize;
    let qrDraw = "";
    if (qrModules) {
      const cellSize = qrSize / qrModules.length;
      qrModules.forEach((row, r) => {
        row.forEach((mod, c) => {
          if (mod) {
            const cx = qrX + c * cellSize;
            const cy = qrY + (qrModules.length - 1 - r) * cellSize; // PDF y-flip
            qrDraw += `${cx.toFixed(1)} ${cy.toFixed(1)} ${cellSize.toFixed(2)} ${cellSize.toFixed(2)} re `;
          }
        });
      });
      qrDraw = `q 0 0 0 rg ${qrDraw}f Q\n`;
      // Label under QR
      qrDraw += `BT /F1 5.5 Tf 0.5 0.5 0.5 rg ${qrX + qrSize / 2 - 30} ${qrY - 10} Td (Scan for piece stickers) Tj ET\n`;
    }
    ...
    const leftCol = [
      ["Batch:", batch.name],
      ["Order:", batch.orderNumber || "N/A"],
      ["Customer:", batch.customerInfo || "N/A"],
      ["Date:", timestamp],
    ];
    const rightCol = [
      ["Template:", tmpl.name],
      ["Size:", tmpl.sizeLabel],
      ["Tray:", `${tmpl.pageW.toFixed(1)}x${tmpl.pageH.toFixed(1)}"`],
    ...
    cs += `BT /F1 ${sm} Tf 0.3 0.3 0.3 rg ${200} ${y} Td (Empty: ${tmpl.totalFrames - filledCount}) Tj ET\n`;
    y -= 18;

    // Photo manifest — two-column compact
    ...
    for (let i = 0; i < half; i++) {
      if (y < 30) { cs += `BT /F1 6 Tf ${lm + 4} ${y} Td (... +${half - i} more) Tj ET\n`; break; }
      const img1 = batch.images[i];
      cs += `BT /F1 6.5 Tf 0 0 0 rg ${lm + 4} ${y} Td (${i + 1}) Tj ET\n`;
      cs += `BT /F1 6.5 Tf ${lm + 18} ${y} Td (${img1.fileName.substring(0, 30).replace(/[()\\]/g, "\\$&")}) Tj ET\n`;
      const j = i + half;
  ...
  // Minimal QR code generator — returns 2D boolean matrix
  function generateQRModules(text) {
    // Use qrcode-generator if loaded, otherwise generate a placeholder
    try {
      if (!window._qrgen) {
        // Load synchronously from CDN — we'll try inline encoding
        // For PDF embedding, we need the module matrix
        // Use a minimal L1 QR encoder
        return minimalQR(text);
      }
    } catch { return null; }
  }

  // Minimal QR Code encoder (Version 1-6, byte mode, EC level L)
  function minimalQR(text) {
    // This is a simplified encoder sufficient for URLs up to ~130 chars
    // For production, consider bundling qrcode-generator npm package
    
    const data = new TextEncoder().encode(text);
    const len = data.length;
    
    // Select version based on data length (byte mode, EC level L)
    const capacities = [0, 17, 32, 53, 78, 106, 134, 154, 192, 230, 271, 321]; // L level byte mode
    let version = 1;
    for (let v = 1; v <= 10; v++) {
      if (len <= capacities[v]) { version = v; break; }
    }
    if (len > capacities[10]) return null; // too long
    ...
    function drawFinderPattern(cx, cy) {
      for (let dy = -4; dy <= 4; dy++) {
        for (let dx = -4; dx <= 4; dx++) {
          ...
          modules[y][x] = dist <= 3 && (dist === 0 || dist === 2 || dist === 3) ? true : (dist === 4 ? false : dist === 1 ? false : false);
          if (dist === 3) modules[y][x] = true;
          if (dist === 2) modules[y][x] = false;
          if (dist === 1) modules[y][x] = false;
          if (dist === 0) modules[y][x] = true;
          isFunction[y][x] = true;
        }
      }
    }
    
    // Simplified: draw finder patterns at corners
    ...
    for (const [cx, cy] of [[3, 3], [size - 4, 3], [3, size - 4]]) {
      for (let dy = -3; dy <= 3; dy++) {
        for (let dx = -3; dx <= 3; dx++) {
          ...
          const dark = adx === 3 || ady === 3 || (adx <= 1 && ady <= 1);
          setMod(cx + dx, cy + dy, dark);
        }
      }
    }
    ...
    // Separators
    for (let i = 0; i < 8; i++) {
      setMod(7, i, false); setMod(i, 7, false);
      setMod(size - 8, i, false); setMod(size - 1 - i, 7, false);
      setMod(7, size - 1 - i, false); setMod(i, size - 8, false);
    }
    ...
    // Fill remaining with simple data pattern (simplified — won't scan perfectly
    // but looks like a QR code in the PDF; for actual scanning, use qrcode-generator)
    // 
    // For a REAL scannable QR, we'll generate it via canvas at runtime in genPDF
    // and embed as an image instead
    
    return null; // Signal to use canvas-based approach instead
  }
  ...
  const genPDF = async (batch) => {
    ...
    try {
      ...
      const filled = allFrames
        .map((f, i) => ({ x: f.x, y: f.y, w: f.w, h: f.h, label: f.label, image: i < batch.images.length ? batch.images[i] : null }))
        .filter(p => p.image);
