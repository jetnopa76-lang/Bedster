...
function buildPDF(trayWIn, trayHIn, placements) {
  ...
  images.forEach((img, i) => {
    ...
    // Apply flip transforms to mirror image for second surface printing
    // flipX: mirror horizontally, flipY: mirror vertically
    let scaleX = drawW, scaleY = drawH, transX = drawX, transY = drawY;
    if (img.flipX) {
      scaleX = -drawW;
      transX = drawX + drawW; // shift right since we're flipping left
    }
    if (img.flipY) {
      scaleY = -drawH;
      transY = drawY + drawH; // shift up since we're flipping down
    }

    cs += `q\n${xPt.toFixed(2)} ${yPt.toFixed(2)} ${fwPt.toFixed(2)} ${fhPt.toFixed(2)} re W n\n`;
    cs += `${scaleX.toFixed(2)} 0 0 ${scaleY.toFixed(2)} ${transX.toFixed(2)} ${transY.toFixed(2)} cm\n/Img${i} Do\nQ\n`;
  });
  cs += 'EMC\n';
  cs += '/OC /MC1 BDC\n/GS_OP gs\n/CS_WI cs\n1 scn\n';
  images.forEach(img => {
